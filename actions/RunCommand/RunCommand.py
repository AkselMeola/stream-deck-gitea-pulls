...
class RunCommand(ActionBase):
    ...
    def execute(self, restart_timer: bool = False):
        ...
        if restart_timer:
            if self.get_is_present() and not settings.get("keep_auto_run_in_background",
                                                          False):  # TODO: Find a better solution
                self.start_timer()
    ...
    def get_config_rows(self):
        ...
        return [entry_row, self.display_output_switch, self.detached_switch, self.auto_run_row,
                self.keep_auto_run_in_background]
    ...
    def run_command(self, command):
        ...
        if self.get_settings().get("detached", True):
            p = multiprocessing.Process(target=subprocess.Popen, args=[command],
                                        kwargs={"shell": True, "start_new_session": True, "stdin": subprocess.DEVNULL,
                                                "stdout": subprocess.DEVNULL, "stderr": subprocess.DEVNULL,
                                                "cwd": os.path.expanduser("~")})
            p.start()
            return ""

        result = subprocess.Popen(command, shell=True, start_new_session=True, text=True, stdout=subprocess.PIPE,
                                  cwd=os.path.expanduser(
